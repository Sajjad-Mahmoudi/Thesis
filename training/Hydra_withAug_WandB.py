...
run = wandb.init(project='Binary_Seg', name='Run_Remote5', id=id, config=configs, save_code=True)
config = wandb.config
...
train_img_generator = data_generator.flow(x_train, seed=seed, batch_size=config.batch_size)
valid_img_generator = data_generator.flow(x_valid, seed=seed, batch_size=config.batch_size)
train_mask_generator = data_generator.flow(y_train, seed=seed, batch_size=config.batch_size)
valid_mask_generator = data_generator.flow(y_valid, seed=seed, batch_size=config.batch_size)
...
checkpoint_filepath = '/data/brussel/106/vsc10602/Run_Remote5/cp-{epoch:03d}.ckpt'
save_weight_every40Epochs = tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_filepath,
                                                                save_weights_only=True,
                                                                save_freq=int(np.ceil(len(x_train)/ config.batch_size).astype(np.uint8) * 100),
                                                                #monitor='val_loss',
                                                                verbose=1
                                                                )

num_train_data = len(x_train)
steps_per_epoch = int(num_train_data / config.batch_size) + 1
num_val_data = len(y_train)
validation_steps = int(num_val_data / config.batch_size) + 1

start_time = time.time()
history = model.fit(train_generator, batch_size=config.batch_size, epochs=num_epoch, validation_data=val_generator, steps_per_epoch=steps_per_epoch,
					initial_epoch=wandb.run.step, validation_steps=validation_steps, callbacks=[save_weight_every40Epochs,
					WandbCallback(generator=val_generator,
