...
namespace BinarySeg
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        public async void UpdateFromFolderCallback(object send, RoutedEventArgs e)
        {
            ...
            {
                TimeSpanTextBox.Text = $"A folder was null";
            }
            ...
            {
                TimeSpanTextBox.Text = $"SRC={srcfiles.Count} does not match DST={gtfiles.Count}!";
            }

            List<double> times = new List<double>();

            TimeSpanTextBox.Text = $"Found {srcfiles.Count} images, warming up...!";


            // Warmup
            var src_init = await ReadImage(srcfiles[0]);
            var input_init = PreprocessImage(src_init);
            _ = await InferenceAsync(input_init);
            ...
            var pics = KnownFolders.PicturesLibrary;
            var file = await pics.CreateFileAsync($"results_{DateTime.Now.Ticks}.txt");
        ...
        private async Task<SoftwareBitmap> ReadImage(StorageFile inputFile)
        {
            ...
            // Only want png files
            if (inputFile.FileType.ToLower() == ".png")
            {
